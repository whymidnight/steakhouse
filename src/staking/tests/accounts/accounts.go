...
func main() {
	_, err := solana.PrivateKeyFromSolanaKeygenFile("/Users/ddigiacomo/SOLANA_KEYS/devnet/sollet.key")
	if err != nil {
		panic(err)
	}
	// nft := new(string)
	...
	t, e := solanarpc.ResolveMintMeta(solana.MustPublicKeyFromBase58(*stakeWallet))
	if !e {
		log.Println("No metadata for", stakeWallet)
	}
	metadata := t.(*typestructs.Metadata)
